...
export const metadata: Metadata = {
  title: "GLORY — Legal · Privacy & Terms",
  description: "The fine print, made clear. GLORY's Privacy Policy and Terms of Service.",
};

type Clause = { n: number; h: string; p?: string; li?: string[] };

const privacy: Clause[] = [
  { n: 1, h: "Who we are", p: "GLORY connects creators with brands to price, sign, and settle distribution deals. This policy explains how we handle personal information when you use our website and app." },
  ...
  { n: 8, h: "Children", p: "GLORY is not intended for anyone under 18 (or the age of majority where you live). We don't knowingly collect data from children." },
  { n: 9, h: "Changes & contact", p: "We'll post updates here and change the “last updated” date. Questions or requests: contact us." },
];

const terms: Clause[] = [
  { n: 1, h: "Acceptance", p: "By creating an account or using GLORY you agree to these Terms. If you're using GLORY for a company, you confirm you're authorised to bind it." },
  ...
  { n: 3, h: "The marketplace", li: [
    ...
    "GLORY is the venue — the agreement for the work is between the creator and the brand.",
  ]},
  { n: 4, h: "Fees & payments", p: "GLORY charges a platform fee on each completed deal, shown before funding. Payments and payouts are handled by Stripe and subject to Stripe's terms. You're responsible for your own taxes." },
  { n: 5, h: "Content & intellectual property", p: "Creators keep ownership of the work they produce. You grant GLORY a limited licence to display your profile and portfolio to operate and promote the marketplace. Licences to the brand for delivered work are set by your deal." },
  ...
  { n: 8, h: "Disclaimers & liability", p: "The service is provided “as is.” To the fullest extent permitted by law, GLORY isn't liable for indirect or consequential losses, and our total liability is limited to the fees you paid us in the 3 months before the claim." },
    ...
    <main id="top" className="bg-[color:var(--color-bg)]">
      ...
      <section className="relative flex min-h-[72svh] items-end overflow-hidden">
        ...
        <div className="relative z-[2] mx-auto w-full max-w-[1160px] px-[clamp(20px,5vw,64px)] pb-[clamp(40px,7vh,80px)]">
          ...
          <p className="mt-5 max-w-[52ch] text-[clamp(15px,1.5vw,18px)] text-[color:var(--color-ink-soft)]">
            The fine print, made plain. How GLORY handles your data, and the terms that govern working inside the platform — written to be read, not skipped.
          </p>
      ...
      <div className="mx-auto mt-[clamp(48px,8vw,88px)] max-w-[820px] px-[clamp(20px,5vw,64px)]">
        <div className="rounded-[10px] border border-[color:var(--hair)] border-l-[3px] border-l-[color:var(--color-gold)] bg-[color:var(--color-panel)] px-[22px] py-[18px] text-sm text-[color:var(--color-ink-soft)]">
          <b className="font-semibold text-[color:var(--color-gold-soft)]">Template notice.</b> This document is a thorough starting point tailored to GLORY, but it is not legal advice. Have a qualified lawyer review and adapt it — especially the governing-law, liability, and payments clauses — before you launch.
        </div>
      </div>
      ...
      <div className="mx-auto max-w-[820px] px-[clamp(20px,5vw,64px)] pb-[clamp(80px,12vw,160px)] pt-[clamp(48px,8vw,96px)]">
        ...
        <section id="terms" className="mt-[clamp(64px,10vw,120px)] scroll-mt-24 border-t border-[color:var(--hair-2)] pt-[clamp(48px,7vw,80px)]">
          ...
          <p className="mt-3.5 text-[17px] text-[color:var(--color-ink-soft)]">The rules of the arena — what you agree to by using GLORY.</p>
          <Clauses items={terms} />
          <a href="#top" className="mt-5 inline-block text-xs uppercase tracking-[0.14em] text-[color:var(--color-ink-mute)] hover:text-[color:var(--color-gold)]">↑ Back to top</a>
        </section>
      </div>
